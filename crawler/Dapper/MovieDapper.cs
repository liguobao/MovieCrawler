    ...
    public class MovieDapper : BaseDapper
    {
        ...
        public int BulkInsert(List<DBMovie> movies)
        {
            ...
            using (IDbConnection dbConnection = GetConnection())
            {
                ...
                var result = dbConnection.Execute(@"INSERT INTO movice
                        (`Name`,`Intro`,`Cover`,`Link`,`Type`,`Resources`,`PublishTime`,`CreateTime`)
                        VALUES (@Name,@Intro,@Cover,@Link,@Type,@Resources,@PublishTime, @CreateTime) ON DUPLICATE KEY UPDATE UpdateTime=now();",
